 ...
 */
package com.worldchip.bbpawphonechat.model;
...
public class HXNotifier {
	...
	private  boolean  has_notification = true;
	...
	public  boolean  isNotification(){
	   has_notification =  MySharePreData.GetBooleanTrueData(appContext, MyComment.CHAT_SP_NAME, MyComment.IS_NOTIFICATION);
	   Log.e(TAG, "---------isNotification-------"+has_notification);
	...
	protected void sendNotification(EMMessage message, boolean isForeground,
			boolean numIncrease) {
		Log.i(TAG, "----HXNotifier-----sendNotification---");
		...
		try {
			...
			Intent intent = new Intent(appContext, PhoneChatActivity.class);
			intent.putExtra("chatto", message.getUserName());
			...
			String bgContentText = msgs[6].replaceFirst("%1",
			...
			if (notificationInfoProvider != null) {
				...
				if (customSummaryBody != null) {
					bgContentText = customSummaryBody;
				}
				// small icon
			...
			// mBuilder.setContentText(bgContentText);
	...
	public void viberateAndPlayTone(EMMessage message) {
		Log.i(TAG, "----HXNotifier-----viberateAndPlayTone---");
		if (message != null) {
			if (EMChatManager.getInstance().isSlientMessage(message)) {
				return;
			}
