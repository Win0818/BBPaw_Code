...
import com.worldchip.bbp.ect.image.utils.ImageSizeUtil.ImageSize;
...
public class ImageLoader
{
	...
	public void loadImage(final String path, final ImageView imageView,
			final boolean isFromNet,final boolean isCache)
	{
		...
		{
			addTask(buildTask(path, imageView, isFromNet,isCache));
		}
		...
		return new Runnable()
		{
			...
			public void run()
			{
				...
				if (isFromNet)
				{
					...
					{
						if (isDiskCacheEnable)// 妫�娴嬫槸鍚﹀紑鍚‖鐩樼紦瀛�
						{
	...
	private Runnable getTask()
	{
		if (mType == Type.FIFO)
		{
			return mTaskQueue.removeFirst();//绉婚櫎骞惰繑鍥炴鍒楄〃鐨勭涓�涓厓绱犮��
	...
	protected void addBitmapToLruCache(String path, Bitmap bm)
	{
		...
		if (bitmapFromLruCache != null) {
			if (bm != null) {
				int lruCacheBitmapSize = bitmapFromLruCache.getRowBytes()*bitmapFromLruCache.getHeight();
				int bitmapSize = bitmapFromLruCache.getRowBytes()*bitmapFromLruCache.getHeight();
				if (lruCacheBitmapSize != bitmapSize) {
